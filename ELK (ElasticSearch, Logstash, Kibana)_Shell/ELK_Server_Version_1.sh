...
+----------------------------------------------------------------------
| Download & Unzip ELK 5.6.3 FOR CentOS 7          1
+----------------------------------------------------------------------
...
elasticsearch=`find /opt -name elasticsearch-5.6.3.tar.gz | wc -l`

if [ $elasticsearch == 1 ]; then   
	#解壓縮「elasticsearch-5.6.3.tar.gz」到「當前目錄」
	tar -xzf elasticsearch-5.6.3.tar.gz -C /opt/
...
logstash=`find /opt -name logstash-5.6.3.tar.gz | wc -l`

if [ $logstash == 1 ]; then
	#解壓縮「logstash-5.6.3.tar.gz」到「當前目錄」
    tar -xzf logstash-5.6.3.tar.gz -C /opt/
...
kibana=`find /opt -name kibana-5.6.3-linux-x86_64.tar.gz | wc -l`

if [ $kibana == 1 ]; then
	#解壓縮「kibana-5.6.3-linux-x86_64.tar.gz」到「當前目錄」
    tar -xzf kibana-5.6.3-linux-x86_64.tar.gz -C /opt/
...
#顯示「位置」
find /opt -name elasticsearch-5.6.3

#顯示「位置」
find /opt -name logstash-5.6.3

#顯示「位置」
find /opt -name kibana-5.6.3-linux-x86_64

fi



#=============判斷「輸入」2 設定「ELK 5.6.3」=========
if [ $select = 2 ]; then


#修改「hostname」
hostnamectl set-hostname elk_5_6_3
...
#賦予「elk」擁有「讀取」權限
chown -R elk:elk /opt/elasticsearch-5.6.3

#最後 面加入「多線程」搜尋
sed -i '$a\thread_pool.search.queue_size: 10000' /opt/elasticsearch-5.6.3/config/elasticsearch.yml
#最後 面加入「transport.host」「network.host」允許「任意」IP 存取「TCP-9200」
sed -i '$a\transport.host: localhost' /opt/elasticsearch-5.6.3/config/elasticsearch.yml
sed -i '$a\network.host: 0.0.0.0' /opt/elasticsearch-5.6.3/config/elasticsearch.yml
...
cat <<EOF >/opt/logstash-5.6.3/config/logstash-conf.yml
...
#最後 面加入「允許」任意連線
sed -i '$a\server.host: "0.0.0.0"' /opt/kibana-5.6.3-linux-x86_64/config/kibana.yml
